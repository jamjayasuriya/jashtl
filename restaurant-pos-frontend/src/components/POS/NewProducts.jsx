...
import { 
  FiSearch, FiFilter, FiEdit, FiTrash2, FiPlus, FiX, FiImage, 
  FiPackage, FiTag, FiTrendingUp, FiRefreshCw
} from 'react-icons/fi';
...
const NewProducts = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    min_stock_level: '',
    status: 'active',
    preparation_area: '', // kitchen, bar, or empty for retail
    image: null
  });
  ...
  const filterProducts = () => {
    ...
    if (filters.preparation_area) {
      if (filters.preparation_area === 'retail') {
        filtered = filtered.filter(product => !product.preparation_area || product.preparation_area === '');
      } else {
        filtered = filtered.filter(product => product.preparation_area === filters.preparation_area);
      }
    }
  ...
  const clearFilters = () => {
    setFilters({ category: '', status: '', preparation_area: '' });
    setSearchTerm('');
  };
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Preparation Area</label>
                <select
                  value={filters.preparation_area}
                  onChange={(e) => setFilters({ ...filters, preparation_area: e.target.value })}
                  className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                  <option value="">All Areas</option>
                  <option value="kitchen">Kitchen Items</option>
                  <option value="bar">Bar Items</option>
                  ...
                  <tr key={product.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        ...
                        {product.preparation_area === 'kitchen' ? 'Kitchen' : 
                         product.preparation_area === 'bar' ? 'Bar' : 'Retail'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="">Retail Item (No Preparation)</option>
